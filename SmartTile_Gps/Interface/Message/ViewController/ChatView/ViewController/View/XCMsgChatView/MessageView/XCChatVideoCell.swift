//
//  XCChatVideoCell.swift
//
//
//  Created by FangLin on 2017/1/15.
...
class XCChatVideoCell: XCChatBaseCell {
...
extension XCChatVideoCell {
    fileprivate func setModel() {
        ...
        // 数据
        chatImgView.sd_setImage(with: URL.init(string: model?.videoCoverUrl ?? ""), completed: nil)
        durationLabel.text = self.getFormatDuration(with: model?.videoDuration)
        durationLabel.sizeToFit()
        
        // 获取缩略图size
        let thumbSize = XCChatMsgDataHelper.shared.getThumbImageSize(model?.videoCoverSize ?? CGSize.zero)
