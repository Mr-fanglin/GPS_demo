...
class XCChatAudioCell: XCChatBaseCell {
...
extension XCChatAudioCell {
    ...
    @objc fileprivate func playAudio() {
        ...
        // 关闭播放
        XCXinChatTools.shared.stopPlayVoice()
        self.resetAudioAnimation()
        
        if XCFileManager.isExists(at: audioPath) {
            XCXinChatTools.shared.playVoice(with: audioPath)
            voiceButton.imageView?.startAnimating()
        } else {
            XCNetworkTools.shared.download(urlStr: model?.audioUrl ?? "", savePath: audioPath, progress: nil, resultBlock: { [unowned self] (_, error) in
                if error != nil {
                    Dprint(error)
                } else {
                    XCXinChatTools.shared.playVoice(with: audioPath)
