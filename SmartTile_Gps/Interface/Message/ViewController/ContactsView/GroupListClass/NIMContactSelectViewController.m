...
- (void)viewDidLoad
{
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 88, self.view.bounds.size.width, self.view.bounds.size.height-64) style:UITableViewStylePlain];
...
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(onCancelBtnClick:)];
    UILabel *titleL = [[UILabel alloc] initWithFrame:CGRectMake(100, 44, self.view.frame.size.width-200, 44)];
    titleL.text = @"Contact";
    ...
    backBtn.frame = CGRectMake(0, 44, 44, 44);
    [backBtn addTarget:self action:@selector(onCancelBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
//        }
//    }
    NSString *members = [_selectecContacts componentsJoinedByString:@","];
    NSString *membersName = [_selectesName componentsJoinedByString:@","];
    [[MessageRequestObject shared] requestGroupCreateWithName:membersName members:members createId:[NIMSDK sharedSDK].loginManager.currentAccount sucBlock:^{
        [self.navigationController popViewControllerAnimated:YES];
    }];
...
- (void)makeData{
    ...
    _selectesName = [NSMutableArray array];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *memberName = [(id<NIMGroupMemberProtocol>)member showName];
    ...
    if([_selectecContacts containsObject:memberId]) {
        [_selectesName removeObject:memberName];
...
//            [self.view makeToast:tip duration:2.0 position:];
        }
        cell.accessoryBtn.selected = NO;
    } else {
        [_selectesName addObject:memberName];
