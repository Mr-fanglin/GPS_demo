...
//  Alamofire封装
//
//  Created by FangLin on 2016/11/14.
...
/// 定义枚举类型
enum XCRequestType: Int {
    case GET
    case POST
}

class XCNetworkTools: AFHTTPSessionManager {
    static let shared: XCNetworkTools = {
...
extension XCNetworkTools {
    // MARK: 请求JSON数据
    // 将成功和失败的回调写在一个逃逸闭包中
    func request(methodType: XCRequestType, urlString: String, parameters: [String : Any], resultBlock:@escaping ([String : Any]?, Error?) -> ()) {
        ...
        if methodType == .GET { // GET
            ...
            post(urlString, parameters: parameters, headers: nil, progress: nil, success: successBlock, failure: failureBlock)
        }
    }
